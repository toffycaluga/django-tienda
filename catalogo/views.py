...
def lista_productos(request):
    ...
    productos = Producto.objects.all().select_related("categoria").prefetch_related("etiquetas")
    if q:
        productos = productos.filter(Q(nombre__icontains=q) | Q(descripcion__icontains=q))
    if categoria_id:
        productos = productos.filter(categoria_id=categoria_id)
    categorias = Categoria.objects.all()
    return render(request, "productos/lista.html", {"productos": productos, "categorias": categorias, "q": q, "categoria_id": categoria_id})
